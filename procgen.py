...
def get_max_value_for_floor(
    ...
    for floor_minimun, value in weighted_chances_by_floor:
        if (floor_minimun >= floor):
            break
        else:
            current_value = value
...
def place_entities(
    ...
    for i in range(number_of_items):
        ...
        if not any(entity.x == x and entity.y == y for entity in dungeon.entities):
            ...
            if (item_chance < 0.7):
                #entity_factories.health_potions.spawn(dungeon, x, y)
...
def generate_dungeon(
    ...
    for r in range(max_rooms):
        ...
        # rectancurlaroom class makes rectangle easier to work with
        new_room = RectangularRoom(x, y, room_width, room_height)

        # run tohrt other rooms and check if they intersect with this one
        if any(new_room.intersects(other_room) for other_room in rooms):
            continue  # this room intersects, so lets go to next
        ...
        if (len(rooms) == 0):
            ...
            # dig tunnel between this room and the previouis one
